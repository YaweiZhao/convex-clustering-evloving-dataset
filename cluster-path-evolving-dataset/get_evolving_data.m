...
if strcmp(s_hyp.noise_type, 'SPARSE NOISE')
    move_data_cluster = 0.02*[ones(s_hyp.n,1) zeros(s_hyp.n,s_hyp.d - 1)] .* (target_position1 - s_hyp.A);
elseif strcmp(s_hyp.noise_type, 'UNIFORM NOISE')
    move_data_cluster = 0.02*ones(s_hyp.n,s_hyp.d) .* (target_position1 - s_hyp.A);
elseif strcmp(s_hyp.noise_type, 'MIXED NOISE')
    move_data_cluster = [0.05*ones(s_hyp.n,1) 0.01*ones(s_hyp.n,s_hyp.d-1)] .* (target_position1 - s_hyp.A);
else
